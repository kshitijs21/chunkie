clear;
%%%%%%%%%% TEST FOR fd_sprime kernel %%%%%%%%%%%
% chnkr = get_chunker(1);
% hh = 1:1:20;
% err = zeros(1,length(hh));
% h0 = 1e-2;
% zz = 1.82;
% Freqk = kernel('helm', 'fd_sprime', zz);
% Ak = chunkermat(chnkr, Freqk); 
% for i = 1:length(hh)
%      h = h0/hh(i);
%      Dp = kernel('helm', 'sprime', zz + h);  
%      Ap = chunkermat(chnkr, Dp);
%      Dm = kernel('helm', 'sprime', zz - h);
...
start1 = tic;
dkernl = kernel('helmholtz','fd_sprime',zk);

clear;
%%%%%%%  Testing the "fd_d" kernel at I don't know which level  %%%%%%%

%%%%%%%%%% TEST FOR THE fd_d kernel %%%%%%%%%%%
...
% deru2 = chnk.helm2d.fmm(eps, zk, srcinfo_kernel, srcinfo_kernel, 'fd_d', qq);
% 
% for ii = 1:length(hh)
%     zp = zk + hh(ii);
%     zn = zk - hh(ii);
...
%%%%%%%  Testing the "fd_d" kernel at layer potentials level  %%%%%%%
...
t2 = toc(start2);
fprintf('nch = %d, t1 = %d, t2 = %d, t2/t1 = %d\n', nch, t1, t2, t2/t1)
...
for ii = 1:length(hhl)
    ...
    err(ii) = norm((Ap*(v.') - An*(v.'))/hhl(ii) - 2*u_eval);
...
% 
figure, loglog(1:1:length(hhl), err, 'k.');      % error is of order 100*h^2

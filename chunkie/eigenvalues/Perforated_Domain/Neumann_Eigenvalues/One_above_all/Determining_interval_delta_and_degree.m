...
[eigval, ddets, ccoef] =  obj_fun_flam(chnkreps, a, b, 24, opts);
...
fprintf('\n');
%%%
save(['Chebyshev_with_' num2str(nholes) '_holes_on_interval_[' num2str(a) ',' num2str(b) '].mat'], 'eigval', 'ddets', 'ccoef', 'chnkreps');
end
...
%
    ...
    if ~isfield(opts, 'zk')
        ...
        for ii=1:ncheb
            zk = xcheb(ii);
            Sp = 2*kernel('helmholtz','d',zk); % ORIGINALLY THE TYPE OF KERNAL WAS 'sprime' BUT I WILL BE CHOOSING 'd' DOUBLE-LAYER AND FOR NEUMANN SIGN WILL BE CHANGED TO + FROM -
...
[zz,~,~,rts] = zeros_fred_opti(func, method, chnkr, opts);
...
%kk = [0.001 0.005 0.01 0.05 0.1 0.5 1];
kk = 1;
%kk = 0.01;
cheb1 = z0-kk;
cheb2 = z0+kk;
...
for ii = 1:length(l)
    ...
    save(['degree_vs_hole_plot_for_' num2str(nh) '_hole_for_' num2str(kk) '_radius_for_obj_fn_' char(func) '_for_' num2str(pts) '_chnkr_npt.mat'], 'cheb1','cheb2', 'tt', 'deg');
end